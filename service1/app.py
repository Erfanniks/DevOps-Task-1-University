...
def log_state_change(old_state, new_state):
    ...
    print(f"DEBUG: Logged state transition: {log_entry}")
...
def manage_state():
    ...
    if request.method == 'PUT':
        ...
        print(f"DEBUG: Received payload for state transition -> {new_state}")
        
        if new_state not in VALID_STATES:
            print(f"DEBUG: Invalid state transition attempt -> {new_state}")
            return "Invalid state", 400
        
        current_state = get_current_state()
        if new_state == current_state:
            print(f"DEBUG: State already set to {current_state}")
            return current_state, 200
        ...
        if new_state == 'INIT':
            ...
            print("DEBUG: State reset to INIT")
        elif new_state == 'SHUTDOWN':
            redis_client.delete('current_state')
            print("DEBUG: System shutdown initiated")
...
def get_run_log():
    ...
    if not logs:
        print("DEBUG: State log is empty")
        return "No state transitions logged yet.", 200
    log_output = '\n'.join([log.decode('utf-8') for log in logs])
    print(f"DEBUG: Retrieved state log -> \n{log_output}")
...
def handle_request():
    ...
    current_state = get_current_state()
    print(f"DEBUG: Handling request in state -> {current_state}")

    if current_state == 'PAUSED':
        print("DEBUG: Service is paused")
        return jsonify({"error": "Service is paused"}), 503

    if current_state != 'RUNNING':
        print("DEBUG: Service is not in running state")
    ...
    if last_request:
        ...
        print(f"DEBUG: Time since last request -> {time_since_last_request} seconds")
...
def stop_service():
    """Stop all containers."""
    print("DEBUG: Stopping all containers...")
...
def metrics():
    ...
    try:
        ...
        print(f"DEBUG: Failed to fetch metrics -> {str(e)}")
